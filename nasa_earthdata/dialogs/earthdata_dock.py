...
class COGDisplayWorker(QThread):
    ...
    def _ensure_netrc(self, netrc_path, auth):
        """Ensure ~/.netrc contains urs.earthdata.nasa.gov credentials.

        GDAL's /vsicurl/ reads .netrc to authenticate HTTP redirects to
        NASA's URS OAuth endpoint.  earthaccess.login(persist=True) usually
        writes this, but if the user authenticated via token or if the file
        is missing, we write it here as a safety net.

        Args:
            netrc_path: Absolute path to the .netrc file.
            auth: The earthaccess Auth object (after successful login).
        """
        host = "urs.earthdata.nasa.gov"

        # Check if .netrc already has Earthdata credentials
        if os.path.exists(netrc_path):
            try:
                import netrc as netrc_mod

                info = netrc_mod.netrc(netrc_path).authenticators(host)
                if info and info[0] and info[2]:
                    return  # already present
            except Exception:
                pass

        # Try to extract username/password from the auth object
        username = getattr(auth, "username", None) or ""
        password = getattr(auth, "password", None) or ""

        if not username or not password:
            self.progress.emit(
                "Warning: Could not write .netrc (no username/password available)"
            )
            return
        ...
        try:
            ...
            self.progress.emit("Updated .netrc with Earthdata credentials")
        except Exception as e:
            self.progress.emit(f"Warning: Could not update .netrc: {e}")
    ...
    def run(self):
        ...
        try:
            ...
            # Ensure .netrc contains Earthdata credentials so GDAL's
            # /vsicurl/ can authenticate via HTTP redirects to URS.
            netrc_path = os.path.expanduser("~/.netrc")
            self._ensure_netrc(netrc_path, auth)

            # Also save session cookies as a fallback for GDAL
...
class EarthdataDockWidget(QDockWidget):
    ...
    def _on_cog_finished(self, results, cookie_file):
        ...
        # GDAL's /vsicurl/ follows the URS OAuth redirect and reads
        # ~/.netrc for credentials – this is the primary auth mechanism.
        netrc_path = os.path.expanduser("~/.netrc")
        gdal.SetConfigOption("GDAL_HTTP_NETRC", "YES")
        gdal.SetConfigOption("GDAL_HTTP_NETRC_FILE", netrc_path)
        # Cookies as supplementary auth (may help after initial redirect)
